...
- [ ] 同一 availability 无法绑两个分组（UK / API 冲突）。
- [ ] 新建分组可选已有时段或现场建时段，列表显示时段与老师。
- [ ] 进组后出现有效订阅；取消订阅后不在组内且不再进入新生成课次名单。
- [ ] 调班后旧订阅结束、新订阅生效，主带 = 新组老师。
- [ ] 未绑分组的时段不出现在正常批量生成结果中。
- [ ] 特殊课与正常占用窗口老师时间重叠时创建失败。
- [ ] 有未来课时段不可硬删。
- [ ] 出勤扣课时写入 ledger，且不可改历史行；冲正新增行。
- [ ] 分组 capacity 与绑定时段 capacity 一致。
