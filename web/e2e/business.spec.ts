...
test('delivery fulfillment displays persisted address and verified reconciliation action', async ({
  page,
}) => {
  const order = {
    id: 'o1',
  ...
  await page.route('**/api/v1/commerce/orders?*', (route) =>
    route.fulfill({ json: { code: 0, data: [order] } }),
  );
  await page.route('**/api/v1/commerce/orders/o1', (route) =>
    route.fulfill({ json: { code: 0, data: order } }),
  );
  await page.route('**/api/v1/commerce/dashboard?*', (route) =>
    route.fulfill({
      json: {
        code: 0,
        data: {
          paidMinor: 2990,
          refundedMinor: 0,
          netReceiptsMinor: 2990,
          definition: '已支付减已完成退款',
        },
      },
    }),
  );
  await page.goto('/commerce');
  await page.getByRole('combobox', { name: '校区', exact: true }).selectOption('1');
  ...
  const request = page.waitForRequest(
    (r) => r.url().endsWith('/commerce/orders/o1/reconcile') && r.method() === 'POST',
  );
  await page.getByRole('button', { name: '主动对账' }).click();
  await request;
...
test('enrolled lead cannot be downgraded or booked for another trial', async ({ page }) => {
  await page.route('**/api/v1/engagement/enquiries?*', (route) =>
    route.fulfill({
      json: {
        code: 0,
        data: [
  ...
  await page.route('**/api/**schedule**', (route) =>
    route.fulfill({ json: { code: 0, data: { days: [] } } }),
  );
  await page.route('**/api/**class-groups**', (route) =>
    route.fulfill({ json: { code: 0, data: { items: [] } } }),
  );
