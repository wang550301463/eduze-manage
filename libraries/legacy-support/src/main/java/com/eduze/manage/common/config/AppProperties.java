...
public class AppProperties {
    ...
    public void validateProdSecrets() {
        ...
        if ("identity-service".equals(environment.getProperty("spring.application.name"))) {
            requireNonBlank(jwt.getSecret(), "eduze.jwt.secret (JWT_SECRET)");
            if (jwt.getSecret().length() < 32) {
                throw new IllegalStateException(
                        "eduze.jwt.secret (JWT_SECRET) must be at least 32 characters in prod");
            }
            ...
            if (WEAK_JWT_SECRETS.contains(normalized) || normalized.contains("change-me")) {
                throw new IllegalStateException(
                        "eduze.jwt.secret (JWT_SECRET) must not use a known weak/default value");
            }
        }
        requireNonBlank(
        ...
        if (dbPassword != null && dbPassword.length() < 8) {
            throw new IllegalStateException(
                    "spring.datasource.password (DB_PASSWORD) must be at least 8 characters in prod");
        }
    }
    ...
    private static void requireNonBlank(String value, String name) {
        if (value == null || value.isBlank()) {
            throw new IllegalStateException(
                    name + " must be non-blank when profile 'prod' is active");
        }
    }
