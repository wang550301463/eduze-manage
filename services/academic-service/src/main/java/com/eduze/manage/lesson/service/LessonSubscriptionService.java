...
public class LessonSubscriptionService {
    ...
    private final LessonStudentService roster;
    ...
    public void delete(Long id) {
        ...
        roster.cancelFuture(entity.getStudentId(), entity.getTeacherAvailabilityId());
    }
    ...
    private List<SubscriptionResponse> toResponses(List<LessonSubscription> subs) {
        ...
        Set<Long> availIds =
                subs.stream()
                        .map(LessonSubscription::getTeacherAvailabilityId)
                        .collect(Collectors.toSet());
        Set<Long> studentIds =
                subs.stream().map(LessonSubscription::getStudentId).collect(Collectors.toSet());
        Set<Long> teacherIds = new HashSet<>();
        subs.forEach(s -> teacherIds.add(s.getTeacherId()));
        Map<Long, TeacherAvailability> availMap =
                availabilityMapper.selectBatchIds(availIds).stream()
    ...
    private SubscriptionResponse toResponse(
            LessonSubscription entity, TeacherAvailability avail, Student student) {
        ...
        UserView teacher = userDirectory.get(entity.getTeacherId());
        return SubscriptionResponse.builder()
                .id(entity.getId())
