...
public class LessonService {
    ...
    private final com.eduze.manage.lesson.mapper.LessonStudentMapper rosterMapper;
    ...
    public List<LessonResponse> list(
        ...
        return lessonAssembler.toResponses(lessonMapper.selectList(query));
    }
    ...
    public LessonResponse create(LessonRequest request) {
        ...
        if (lesson.getClassGroupId() != null) {
            var studentIds =
                    jdbc.queryForList(
                            "SELECT g.student_id FROM t_student_class_group g JOIN t_student s ON s.id=g.student_id AND s.tenant_id=g.tenant_id WHERE g.tenant_id=? AND g.class_group_id=? AND g.deleted_at=0 AND g.left_at IS NULL AND s.deleted_at=0 AND s.branch_id=?",
                            Long.class,
                            lesson.getTenantId(),
                            lesson.getClassGroupId(),
                            lesson.getBranchId());
            for (Long studentId : studentIds) {
                var row = new com.eduze.manage.lesson.domain.LessonStudent();
                row.setTenantId(lesson.getTenantId());
                row.setBranchId(lesson.getBranchId());
                row.setLessonId(lesson.getId());
                row.setStudentId(studentId);
                row.setSource("SUBSCRIPTION");
                row.setStatus("BOOKED");
    ...
    public LessonResponse reschedule(Long id, RescheduleRequest request) {
        ...
        if (lessonMapper.updateById(lesson) != 1) {
            throw new BizException(ErrorCode.CONFLICT, "课次已被其他人修改");
        }
        events.lessonChanged(lesson);
