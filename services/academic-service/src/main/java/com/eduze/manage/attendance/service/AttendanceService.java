...
public class AttendanceService {
    ...
    public TodayRosterResponse todayRoster(Long branchId, String period, LocalDate date) {
        ...
        for (Map<String, Object> row : rows) {
            ...
            Integer status =
                    row.get("status") != null ? ((Number) row.get("status")).intValue() : null;
            LocalDate lessonDate = toLocalDateTime(row.get("start_at")).toLocalDate();
            if (status == null
                    && attendanceSupport.hasApprovedLeave(
                            studentId, ((Number) row.get("lesson_id")).longValue(), lessonDate)) {
                status = AttendanceStatus.LEAVE.getCode();
            }
            if (status != null
                    && (status == AttendanceStatus.CHECKED_IN.getCode()
            ...
            items.add(
                    RosterItem.builder()
                            ...
                            .attendanceId(
                                    row.get("attendance_id") != null
                                            ? ((Number) row.get("attendance_id")).longValue()
                                            : null)
                            .status(status)
                            .statusLabel(AttendanceSupport.statusLabel(status))
                            .checkInAt(
                                    row.get("check_in_at") != null
                                            ? toLocalDateTime(row.get("check_in_at"))
                                            : null)
                            .checkOutAt(
                                    row.get("check_out_at") != null
                                            ? toLocalDateTime(row.get("check_out_at"))
                                            : null)
                            .build());
        }
    ...
    public AttendanceResponse checkIn(CheckInRequest request) {
        ...
        var lockedLesson =
                rosterCapacity.lock(
                        TenantContext.getTenantId().toString(), lesson.getId().toString());
        if ("CANCELLED".equals(lockedLesson.get("status")))
            throw new BizException(ErrorCode.CONFLICT, "课次已取消");
        ...
        Integer inRoster =
                jdbcTemplate.queryForObject(
                        ...
                        student.getId());
        if (inRoster == 0) {
            throw new BizException(ErrorCode.FORBIDDEN, "学员不在课次名单中");
        }
        if ("CANCELLED".equals(lesson.getStatus())) {
            throw new BizException(ErrorCode.CONFLICT, "课次已取消");
        }

        if (attendanceSupport.hasApprovedLeave(
                student.getId(), lesson.getId(), lesson.getStartAt().toLocalDate())) {
            Attendance existing = findAttendance(lesson.getId(), student.getId());
            if (existing != null) {
                if (existing.getStatus() != AttendanceStatus.LEAVE.getCode())
                    throw new BizException(ErrorCode.CONFLICT, "已有考勤记录，请先显式纠正后再处理请假");
            ...
            Attendance leaveRow =
                    buildAttendance(lesson, student, AttendanceStatus.LEAVE.getCode(), null);
        ...
        Attendance attendance =
                buildAttendance(
                        lesson,
                        student,
                        AttendanceStatus.CHECKED_IN.getCode(),
    ...
    public AttendanceResponse checkOut(CheckOutRequest request) {
        ...
        if (!abnormal
                && request.getGuardianId() != null
                && !attendanceSupport.isAuthorizedPickup(
                        student.getId(), request.getGuardianId())) {
            abnormal = true;
        }
        pickupService.recordPickupInternal(
                attendance,
