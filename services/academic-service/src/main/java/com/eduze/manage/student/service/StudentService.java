...
import com.eduze.manage.directory.StageDirectory;
import com.eduze.manage.directory.StageView;
...
public class StudentService {
    ...
    private final StageDirectory stageDirectory;
    ...
    public Page<StudentResponse> list(
            ...
            wrapper.and(
                    w ->
                            w.like(Student::getName, keyword)
                                    .or()
                                    .like(Student::getEnrollNo, keyword));
        }
        if (branchId != null) {
        ...
        userDirectory.batch(
                result.getRecords().stream()
                        .map(Student::getMentorTeacherId)
                        .filter(java.util.Objects::nonNull)
                        .toList());
        stageDirectory.batch(
                result.getRecords().stream()
                        .map(Student::getCurrentStageId)
                        .filter(java.util.Objects::nonNull)
                        .toList());
        Map<Long, String> branchNames = loadBranchNames(result.getRecords());
        Map<Long, Integer> balances =
                packageBalanceHelper.sumRemaining(
    ...
    private void createInitialSubscription(
            Student student, StudentRequest.InitialSubscription sub) {
    ...
    private List<Long> findStudentIdsByMaxRemaining(int maxRemaining) {
        ...
        return jdbcTemplate.queryForList(
                ...
                GROUP BY student_id
                HAVING COALESCE(SUM(remaining_lessons-frozen_lessons), 0) <= ?
                """,
                Long.class,
                tenantId,
                maxRemaining);
    ...
    private Map<Long, List<StudentResponse.ClassGroupRef>> loadClassGroups(List<Long> studentIds) {
        ...
        for (Map<String, Object> row : rows) {
            ...
            result.computeIfAbsent(studentId, k -> new java.util.ArrayList<>())
                    .add(
                            StudentResponse.ClassGroupRef.builder()
    ...
    private Map<Long, String> loadBranchNames(List<Student> students) {
        ...
        List<BranchView> branches = branchDirectory.batch(branchIds);
        return branches.stream()
                .collect(Collectors.toMap(BranchView::getId, BranchView::getName, (a, b) -> a));
    }
    ...
    private StudentResponse toResponse(
        ...
        if (student.getMentorTeacherId() != null) {
            UserView mentor = userDirectory.get(student.getMentorTeacherId());
            if (mentor != null) {
        ...
        if (student.getCurrentStageId() != null) {
            StageView stage = stageDirectory.get(student.getCurrentStageId());
            if (stage != null) {
