...
public class ConflictService {
    ...
    public ConflictReport check(LessonDraft draft) {
        LessonResponse teacher =
                draft.teacherId() != null
                        ...
                        : null;
        LessonResponse classRoom =
                draft.classRoomId() != null
                        ? findOverlap(
                                draft,
                                Wrappers.<Lesson>lambdaQuery()
                                        .eq(Lesson::getClassRoomId, draft.classRoomId()))
                        : null;
        LessonResponse classGroup =
                draft.classGroupId() != null
                        ? findOverlap(
                                draft,
                                Wrappers.<Lesson>lambdaQuery()
                                        .eq(Lesson::getClassGroupId, draft.classGroupId()))
                        : null;
    ...
    private LessonResponse findBoundAvailabilityOverlap(LessonDraft draft) {
        ...
        List<ClassGroup> boundGroups =
                classGroupMapper.selectList(
        ...
        List<TeacherAvailability> avails =
                teacherAvailabilityMapper.selectList(
                ...
                return LessonResponse.builder()
                        ...
                        .startAt(
                                draft.startAt()
                                        .toLocalDate()
                                        .atStartOfDay()
                                        .plusMinutes(avail.getStartMinute()))
                        .endAt(
                                draft.startAt()
                                        .toLocalDate()
                                        .atStartOfDay()
                                        .plusMinutes(avail.getEndMinute()))
                        .build();
            }
        }
        return null;
    ...
    private LessonResponse findOverlap(
            LessonDraft draft,
            com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<Lesson> scoped) {
        Lesson conflict =
                lessonMapper.selectOne(
                        scoped.eq(Lesson::getTenantId, TenantContext.getTenantId())
