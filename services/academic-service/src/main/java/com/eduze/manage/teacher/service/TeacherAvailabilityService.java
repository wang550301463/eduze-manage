...
public class TeacherAvailabilityService {
    ...
    public List<TeacherAvailabilityResponse> list(Long teacherId) {
        LambdaQueryWrapper<TeacherAvailability> wrapper =
                new LambdaQueryWrapper<TeacherAvailability>()
    ...
    public List<TeacherAvailabilityResponse> listUnbound(Long branchId) {
        ...
        List<TeacherAvailability> avails =
                mapper.selectList(
                        new LambdaQueryWrapper<TeacherAvailability>()
        ...
        Set<Long> availIds =
                avails.stream().map(TeacherAvailability::getId).collect(Collectors.toSet());
        Set<Long> boundIds =
                classGroupMapper
                        .selectList(
    ...
    public boolean hasOverlap(
            Long teacherId, Integer dayOfWeek, Integer startMinute, Integer endMinute) {
        LambdaQueryWrapper<TeacherAvailability> wrapper =
                new LambdaQueryWrapper<TeacherAvailability>()
                        .eq(TeacherAvailability::getTeacherId, teacherId)
        ...
        return mapper.selectList(wrapper).stream()
                .anyMatch(a -> overlap(a, startMinute, endMinute));
    }
    ...
    private List<TeacherAvailabilityResponse> toResponses(List<TeacherAvailability> list) {
        ...
        Set<Long> teacherIds =
                list.stream().map(TeacherAvailability::getTeacherId).collect(Collectors.toSet());
        Map<Long, String> teacherNames =
                userDirectory.batch(teacherIds).stream()
                        .collect(Collectors.toMap(UserView::getId, UserView::getName, (a, b) -> a));
        Set<Long> availIds =
                list.stream().map(TeacherAvailability::getId).collect(Collectors.toSet());
        ...
        return list.stream()
                .map(
                        e ->
                                toResponse(
                                        e,
                                        teacherNames.get(e.getTeacherId()),
                                        boundGroupByAvail.get(e.getId())))
                .toList();
    }

    private TeacherAvailabilityResponse toResponse(
            TeacherAvailability e, String teacherName, Long boundClassGroupId) {
        if (teacherName == null && e.getTeacherId() != null) {
            UserView t = userDirectory.get(e.getTeacherId());
