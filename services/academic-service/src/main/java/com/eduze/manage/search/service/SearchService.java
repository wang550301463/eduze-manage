...
public class SearchService {
    ...
    private static final DateTimeFormatter LESSON_TIME_FMT =
    ...
    private List<SearchHit> searchGuardians(String keyword) {
        ...
        List<Guardian> guardians =
                guardianMapper.selectList(
                        Wrappers.<Guardian>lambdaQuery()
                                .and(
                                        w ->
                                                w.like(Guardian::getName, keyword)
                                                        .or()
                                                        .like(Guardian::getPhone, keyword))
        ...
        for (Guardian g : guardians) {
            ...
            List<Long> studentIds =
                    relations.stream().map(StudentGuardianRelation::getStudentId).toList();
            List<Student> students = studentMapper.selectBatchIds(studentIds);
            if (branchScope != null) {
                students =
                        students.stream()
                                .filter(s -> branchScope.contains(s.getBranchId()))
                                .toList();
                if (students.isEmpty()) {
                    continue;
                }
    ...
    private List<SearchHit> searchLessons(String keyword, List<Long> branchScope) {
        ...
        List<ClassGroup> groups =
                classGroupMapper.selectList(groupWrapper.last("LIMIT " + PER_TYPE_LIMIT));
        if (groups.isEmpty()) {
            return List.of();
        }
            ...
            for (Lesson lesson : lessons) {
                String time =
                        lesson.getStartAt() != null
                                ? lesson.getStartAt().format(LESSON_TIME_FMT)
                                : "";
                hits.add(
                        SearchHit.builder()
                                .type("lesson")
    ...
    private Map<Long, String> loadBranchNames(Set<Long> branchIds) {
        ...
        return branchDirectory.batch(branchIds).stream()
