...
public class StudentMentorService {
    ...
    /** 校验 mentor 是否合法（同 tenant + branch + TEACHER 角色 + 启用）。返回 mentor UserView 实体。 */
    public UserView requireMentor(Long teacherId, Long branchId) {
        if (teacherId == null) {
            throw new BizException(ErrorCode.VALIDATION_FAILED, "主带老师必填");
        }
        UserView user = userDirectory.get(teacherId);
        if (user == null
                || !user.getRoles().contains("TEACHER")
                || !Integer.valueOf(1).equals(user.getStatus())) {
            throw new BizException(ErrorCode.VALIDATION_FAILED, "主带老师不存在或不是启用的老师");
        }
        if (branchId != null && !user.getBranchIds().contains(branchId)) {
    ...
    public void recordHistory(
            Long studentId, Long branchId, Long fromTeacherId, Long toTeacherId, String reason) {
    ...
    public void changeMentor(Long studentId, AssignMentorRequest req) {
        ...
        if (!keep && oldTeacherId != null) {
            LambdaUpdateWrapper<LessonSubscription> u =
                    new LambdaUpdateWrapper<LessonSubscription>()
    ...
    public List<MentorHistoryResponse> history(Long studentId) {
        List<StudentMentorHistory> histories =
        ...
        Map<Long, String> nameById =
                teacherIds.isEmpty()
                        ...
                        : userDirectory.batch(teacherIds).stream()
                                .collect(
                                        Collectors.toMap(
                                                UserView::getId, UserView::getName, (a, b) -> a));
        return histories.stream()
                .map(
                        h ->
                                MentorHistoryResponse.builder()
                                        .id(h.getId())
                                        .fromTeacherId(h.getFromTeacherId())
                                        .fromTeacherName(
                                                h.getFromTeacherId() == null
                                                        ? null
                                                        : nameById.get(h.getFromTeacherId()))
