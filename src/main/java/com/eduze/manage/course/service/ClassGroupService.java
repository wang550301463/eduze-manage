...
public class ClassGroupService {
    ...
    public ClassGroupResponse update(Long id, ClassGroupRequest request) {
        ...
        if (newAvailId != null && !Objects.equals(newAvailId, oldAvailId)) {
            TeacherAvailability newAvail = requireBindableAvailability(newAvailId, request.getBranchId());
    ...
    public ClassGroup findByAvailabilityId(Long teacherAvailabilityId) {
        ...
        return classGroupMapper.selectOne(Wrappers.<ClassGroup>lambdaQuery()
                .eq(ClassGroup::getTenantId, TenantContext.getTenantId())
    ...
    private Long resolveAvailabilityIdForCreate(ClassGroupRequest request) {
        if (request.getNestedAvailability() != null) {
            ...
            if (request.getTeacherAvailabilityId() != null) {
                throw new BizException(ErrorCode.VALIDATION_FAILED, "teacherAvailabilityId 与 nestedAvailability 只能二选一");
            }
        ...
        if (request.getTeacherAvailabilityId() == null) {
            throw new BizException(ErrorCode.VALIDATION_FAILED, "新建分组须绑定老师可用时段（teacherAvailabilityId 或 nestedAvailability）");
        }
    ...
    private void migrateSubscriptionsAndMentors(ClassGroup group, Long oldAvailId, TeacherAvailability newAvail) {
        List<StudentClassGroup> members = studentClassGroupMapper.selectList(Wrappers.<StudentClassGroup>lambdaQuery()
                .eq(StudentClassGroup::getTenantId, TenantContext.getTenantId())
        ...
        for (StudentClassGroup membership : members) {
            ...
            if (oldAvailId != null) {
                List<LessonSubscription> oldSubs = lessonSubscriptionMapper.selectList(
                        Wrappers.<LessonSubscription>lambdaQuery()
                                ...
                                .eq(LessonSubscription::getTeacherAvailabilityId, oldAvailId)
            ...
            if (student != null && !Objects.equals(student.getMentorTeacherId(), newAvail.getTeacherId())) {
