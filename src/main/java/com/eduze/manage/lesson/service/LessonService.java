...
public class LessonService {
    ...
    public List<LessonResponse> list(
            Long branchId, Long classGroupId, Long teacherId, LocalDateTime from, LocalDateTime to) {
        if (branchId != null) {
            branchAccessGuard.requireBranchAccess(branchId);
        }
        var query = Wrappers.<Lesson>lambdaQuery()
    ...
    public LessonResponse reschedule(Long id, RescheduleRequest request) {
        ...
        Long teacherId = request.getTeacherId() != null ? request.getTeacherId() : lesson.getTeacherId();
        Long classRoomId = request.getClassRoomId() != null ? request.getClassRoomId() : lesson.getClassRoomId();
    ...
    public List<LessonChangeLogResponse> changeLogs(Long lessonId) {
        ...
        return lessonChangeLogMapper.selectList(Wrappers.<LessonChangeLog>lambdaQuery()
                ...
                .stream()
                .map(log -> LessonChangeLogResponse.builder()
                        .id(log.getId())
    ...
    private void writeChangeLog(
            Long lessonId, String changeType, Map<String, Object> before, Map<String, Object> after, String reason) {
