...
public class LessonStudentService {
    ...
    public List<LessonStudentResponse> list(Long lessonId) {
        List<LessonStudent> rows = lessonStudentMapper.selectList(
                new LambdaQueryWrapper<LessonStudent>().eq(LessonStudent::getLessonId, lessonId));
        if (rows.isEmpty()) return List.of();
        Set<Long> studentIds = rows.stream().map(LessonStudent::getStudentId).collect(Collectors.toSet());
        ...
        return rows.stream().map(r -> LessonStudentResponse.builder()
                        .id(r.getId())
    ...
    public void removeStudent(Long lessonId, Long studentId) {
        LessonStudent existing = lessonStudentMapper.selectOne(
