...
public class ScheduleViewService {
    ...
    public TeacherScheduleResponse byTeacher(Long branchId, LocalDate weekStart) {
        ...
        Set<Long> teacherIds = teacherRows.stream()
        ...
        Map<Long, User> userById = userMapper.selectBatchIds(teacherIds).stream()
                .collect(Collectors.toMap(User::getId, u -> u));
        Map<Long, ClassRoom> roomById = lessons.stream()
                .map(Lesson::getClassRoomId)
                .filter(java.util.Objects::nonNull)
                .distinct()
                .collect(Collectors.toMap(id -> id, id -> classRoomMapper.selectById(id), (a, b) -> a));
        Map<Long, TeacherAvailability> availById = lessons.stream()
                .map(Lesson::getTeacherAvailabilityId)
                .filter(java.util.Objects::nonNull)
                .distinct()
                .collect(Collectors.toMap(id -> id, id -> availabilityMapper.selectById(id), (a, b) -> a));

        // 3. 学员数：按 lessonId GROUP COUNT
        Map<Long, Integer> studentCountByLesson = new HashMap<>();
        if (!lessons.isEmpty()) {
            List<Long> lessonIds = lessons.stream().map(Lesson::getId).toList();
            List<LessonStudent> roster = lessonStudentMapper.selectList(
        ...
        // 4. 按老师分组
        Map<Long, List<Lesson>> lessonsByTeacher = lessons.stream()
                .filter(l -> l.getTeacherId() != null)
                .collect(Collectors.groupingBy(Lesson::getTeacherId));

        List<TeacherScheduleResponse.TeacherColumn> columns = teacherRows.stream()
                .map(r -> buildColumn(
                        ((Number) r.get("id")).longValue(),
                        (String) r.get("name"),
                        r.get("branch_id") == null ? null : ((Number) r.get("branch_id")).longValue(),
                        lessonsByTeacher,
                        roomById,
                        availById,
    ...
    public TeacherScheduleResponse myWeek(Long teacherId, LocalDate weekStart) {
        ...
        TeacherScheduleResponse.TeacherColumn col = buildColumn(
                teacherId, teacher.getName(), teacher.getBranchId(),
                grouped, roomById, availById, studentCountByLesson);
    ...
    private TeacherScheduleResponse.TeacherColumn buildColumn(
        ...
        List<TeacherScheduleResponse.LessonCell> cells = lessons.stream()
    ...
    private TeacherScheduleResponse.LessonCell toCell(
        ...
        TeacherAvailability avail = l.getTeacherAvailabilityId() == null ? null : availById.get(l.getTeacherAvailabilityId());
