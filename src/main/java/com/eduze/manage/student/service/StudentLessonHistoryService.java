...
public class StudentLessonHistoryService {
    ...
    public void recordFromCheckIn(Student student, Lesson lesson, Attendance attendance) {
        ...
        ClassGroup classGroup = lesson.getClassGroupId() != null
                ? classGroupMapper.selectById(lesson.getClassGroupId())
                : null;
        Course course = classGroup != null && classGroup.getCourseId() != null
                ? courseMapper.selectById(classGroup.getCourseId())
                : null;
        User teacher = lesson.getTeacherId() != null ? userMapper.selectById(lesson.getTeacherId()) : null;
        ClassRoom classRoom =
                lesson.getClassRoomId() != null ? classRoomMapper.selectById(lesson.getClassRoomId()) : null;

        Integer minutes = null;
        if (lesson.getStartAt() != null && lesson.getEndAt() != null) {
            minutes = (int) Math.max(1, Duration.between(lesson.getStartAt(), lesson.getEndAt()).toMinutes());
        }

        LocalDateTime occurredAt = attendance != null && attendance.getCheckInAt() != null
        ...
        row.setBranchId(student.getBranchId() != null ? student.getBranchId() : lesson.getBranchId());
        ...
        row.setCourseId(course != null ? course.getId() : (classGroup != null ? classGroup.getCourseId() : null));
    ...
    public Page<StudentLessonHistoryResponse> list(
        ...
        Page<StudentLessonHistory> result = historyMapper.selectPage(
                new Page<>(page, size),
                Wrappers.<StudentLessonHistory>lambdaQuery()
                        .eq(StudentLessonHistory::getTenantId, TenantContext.getTenantId())
                        .eq(studentId != null, StudentLessonHistory::getStudentId, studentId)
