...
public class CoursePackageService {
    ...
    private CoursePackageResponse toResponse(CoursePackage pkg) {
        boolean expired = pkg.getExpireDate() != null && pkg.getExpireDate().isBefore(LocalDate.now());
        int remaining = expired ? 0 : (pkg.getRemainingLessons() != null ? pkg.getRemainingLessons() : 0);
