...
public class StudentImportService {
    ...
    public void writeTemplate(HttpServletResponse response) throws IOException {
        ...
        EasyExcel.write(response.getOutputStream(), StudentImportRow.class).sheet("学员导入").doWrite(List.of());
    }

    public StudentImportResult importExcel(MultipartFile file) throws IOException {
        Map<String, Long> branchByCode = branchMapper
                .selectList(Wrappers.<Branch>lambdaQuery().eq(Branch::getTenantId, TenantContext.getTenantId()))
                .stream()
                .collect(Collectors.toMap(Branch::getCode, Branch::getId, (a, b) -> a));

        List<StudentImportRow> rows = new ArrayList<>();
        try (InputStream in = file.getInputStream()) {
            EasyExcel.read(in, StudentImportRow.class, new ReadListener<StudentImportRow>() {
                        @Override
        ...
        for (StudentImportRow row : rows) {
            ...
            if (!errors.isEmpty()) {
                failures.add(StudentImportResult.ImportFailure.builder()
                        .rowIndex(rowNum)
                        .errors(errors)
                        .build());
                continue;
            ...
            try {
                ...
                failures.add(StudentImportResult.ImportFailure.builder()
                        .rowIndex(rowNum)
                        .errors(List.of(StudentImportResult.FieldError.builder()
                                .column("_")
                                .message(ex.getMessage() != null ? ex.getMessage() : "导入失败")
                                .build()))
                        .build());
            }
        }
    ...
    private List<StudentImportResult.FieldError> validateRow(StudentImportRow row, Map<String, Long> branchByCode) {
        ...
        if (!StringUtils.hasText(row.getBranchCode()) || !branchByCode.containsKey(row.getBranchCode().trim())) {
    ...
    private void importRow(StudentImportRow row, Map<String, Long> branchByCode) {
        ...
        if (StringUtils.hasText(row.getGuardianPhone())) {
            ...
            guardian.setName(
                    StringUtils.hasText(row.getGuardianName()) ? row.getGuardianName().trim() : "家长");
    ...
    private StudentImportResult.FieldError field(String column, String message) {
        return StudentImportResult.FieldError.builder()
                .column(column)
                .message(message)
                .build();
    }
