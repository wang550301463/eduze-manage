...
public class AuthService {
    ...
    private LoginResponse issueTokens(CustomUserDetails details) {
        ...
        List<String> auths = details.getAuthorities().stream()
                .map(a -> a.getAuthority())
                .toList();
        List<String> roles = List.copyOf(details.getRoleCodes());

        String accessToken = jwtService.generateAccess(
                details.getUserId(), accessJti, details.getTenantId(), tokenVersion, branchIds, roles, auths);
        String refreshToken = jwtService.generateRefresh(
                details.getUserId(), refreshJti, details.getTenantId(), tokenVersion, branchIds, roles, auths);
        refreshTokenStore.store(
                refreshJti, details.getUserId(), Instant.now().plusSeconds(jwtService.getRefreshTtlSeconds()));
