...
public class AttendanceStatService {
    ...
    public ClassGroupAttendanceStat classGroupStat(Long classGroupId, LocalDate from, LocalDate to) {
        LocalDate[] range = defaultRange(from, to);
        StudentAttendanceStat stat = aggregate(countRows(
                """
                SELECT a.status, COUNT(*) AS cnt
                FROM t_attendance a
    ...
    public BranchAttendanceStat branchStat(Long branchId, LocalDate from, LocalDate to) {
        LocalDate[] range = defaultRange(from, to);
        StudentAttendanceStat overall = aggregate(countRows(
                """
                SELECT a.status, COUNT(*) AS cnt
                FROM t_attendance a
    ...
    public Map<String, Object> dashboardKpis(Long branchId) {
        ...
        Integer weekAbsent = jdbcTemplate.queryForObject(
                """
        ...
        Integer weekNewStudents = jdbcTemplate.queryForObject(
                """
        ...
        Integer weekLessons = jdbcTemplate.queryForObject(
                """
