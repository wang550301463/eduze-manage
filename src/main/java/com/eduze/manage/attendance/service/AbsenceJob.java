...
public class AbsenceJob {
    ...
    private static final String LOCK_KEY = "lock:absence-job";
    ...
    public void runInternal() {
        ...
        List<Map<String, Object>> lessons = jdbcTemplate.queryForList(
                """
                SELECT id, tenant_id, branch_id, class_group_id, end_at
                FROM t_lesson
    ...
    private void processLesson(Map<String, Object> lesson) {
        ...
        LocalDateTime endAt = toLocalDateTime(lesson.get("end_at"));
        Object classGroupRaw = lesson.get("class_group_id");

        List<Long> studentIds;
        if (classGroupRaw == null) {
            studentIds = jdbcTemplate.queryForList(
                    """
                    SELECT student_id FROM t_lesson_student
                    WHERE lesson_id = ? AND deleted_at = 0 AND status = 'BOOKED'
                      AND (removed_at IS NULL)
                    """,
                    Long.class,
                    lessonId);
        } else {
            Long classGroupId = ((Number) classGroupRaw).longValue();
            studentIds = jdbcTemplate.queryForList(
                    """
                    SELECT student_id FROM t_student_class_group
                    WHERE class_group_id = ? AND deleted_at = 0 AND left_at IS NULL
                    """,
                    Long.class,
                    classGroupId);
        }
        ...
        for (Long studentId : studentIds) {
            Long exists = attendanceMapper.selectCount(Wrappers.<Attendance>lambdaQuery()
                    .eq(Attendance::getLessonId, lessonId)
                    .eq(Attendance::getStudentId, studentId));
            if (exists != null && exists > 0) {
                continue;
            }
            if (attendanceSupport.hasApprovedLeave(studentId, endAt.toLocalDate())) {
                continue;
            }
