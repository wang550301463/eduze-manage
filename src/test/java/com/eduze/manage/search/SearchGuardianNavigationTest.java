...
class SearchGuardianNavigationTest {
    ...
    void linkedGuardianOpensAnAuthorizedStudentWithoutRoundingIds() {
        var user = new CustomUserDetails(1L, 1L, "teacher", "老师", "", List.of(22L),
                Set.of("TEACHER"), List.of("student:read"), 1, true);
        ...
        var students = stub(StudentMapper.class, Map.of("selectBatchIds", List.of(outsideScope, insideScope)));
        var guardians = stub(GuardianMapper.class, Map.of("selectList", List.of(guardian)));
        var relations = stub(StudentGuardianRelationMapper.class, Map.of("selectList", List.of(relation)));
        var service = new SearchService(students, guardians, relations,
                stub(BranchMapper.class, Map.of()), stub(ClassGroupMapper.class, Map.of()),
                stub(LessonMapper.class, Map.of()), new BranchAccessGuard());

        var hits = service.search("林", Set.of("guardian"), 20);
        assertEquals(1, hits.size());
        assertEquals("/students?openId=9007199254740993124&openGuardianId=9007199254740993123",
                hits.get(0).getUrl());
    ...
    void usersWithoutBranchesCannotReadLinkedOrUnlinkedGuardianContacts(boolean linked) {
        var user = new CustomUserDetails(1L, 1L, "teacher", "老师", "", List.of(),
                Set.of("TEACHER"), List.of("search:read"), 1, true);
        SecurityContextHolder.getContext().setAuthentication(
                new UsernamePasswordAuthenticationToken(user, null, user.getAuthorities()));
        var guardian = new Guardian();
        guardian.setId(91L);
        ...
        var service = new SearchService(
                ...
                stub(StudentGuardianRelationMapper.class, Map.of("selectList", linked ? List.of(relation) : List.of())),
                stub(BranchMapper.class, Map.of()), stub(ClassGroupMapper.class, Map.of()),
                stub(LessonMapper.class, Map.of()), new BranchAccessGuard());
    ...
    private static <T> T stub(Class<T> mapper, Map<String, Object> responses) {
        return mapper.cast(Proxy.newProxyInstance(mapper.getClassLoader(), new Class<?>[] {mapper},
                (proxy, method, args) -> {
