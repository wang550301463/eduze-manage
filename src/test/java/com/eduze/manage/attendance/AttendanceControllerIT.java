...
class AttendanceControllerIT extends AbstractITContainerTest {
    ...
    void todayRoster_andCheckIn() throws Exception {
        ...
        long lessonId = AttendanceFixture.seedLessonWithStudent(jdbcTemplate, start, start.plusHours(2));
    ...
    void qrCheckIn() throws Exception {
        ...
        var qrResult = mockMvc.perform(post("/api/guardians/" + guardianId + "/qr")
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andReturn();
