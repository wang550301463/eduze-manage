...
class LeaveServiceIT extends AbstractITContainerTest {
    ...
    void approveLeave_updatesExistingAttendance() throws Exception {
        ...
        var leaveResult = mockMvc.perform(post("/api/leaves")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(String.format(
                                """
                ...
                .andExpect(status().isOk())
                .andReturn();
        long leaveId = objectMapper
                .readTree(leaveResult.getResponse().getContentAsString())
                .path("data")
                .path("id")
                .asLong();

        mockMvc.perform(post("/api/leaves/" + leaveId + "/approve").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
