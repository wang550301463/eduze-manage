...
class StudentImportIT extends AbstractITContainerTest {
    ...
    void import_partialSuccess() throws Exception {
        ...
        MockMultipartFile file = new MockMultipartFile("file", "students.xlsx", "application/vnd.ms-excel", bytes);
        ...
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM t_student WHERE enroll_no LIKE 'IMP-%'", Integer.class);
        assertEquals(3, count);

        Long mentor = jdbcTemplate.queryForObject(
                "SELECT mentor_teacher_id FROM t_student WHERE enroll_no = 'IMP-1'", Long.class);
    ...
    void import_missingMentor_reportsFailure() throws Exception {
        ...
        mockMvc.perform(multipart("/api/students/import").file(file).with(api.bearer(token)))
                ...
                .andExpect(jsonPath("$.data.failures[0].errors[0].column").value("mentorTeacherId"));
    }
