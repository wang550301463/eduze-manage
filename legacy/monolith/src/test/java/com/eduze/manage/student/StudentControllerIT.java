...
class StudentControllerIT extends AbstractITContainerTest {
    ...
    void branchScope_nonSuperAdminCannotSeeOtherBranch() throws Exception {
        ...
        jdbcTemplate.update(
                "INSERT IGNORE INTO t_user_branch (id, tenant_id, user_id, branch_id) VALUES (9102, 1, 1101, 2)");
        ...
        mockMvc.perform(
                        get("/api/students")
                                .param("keyword", "BR2-001")
                                .with(api.bearer(advisorToken)))
                .andExpect(status().isOk())
