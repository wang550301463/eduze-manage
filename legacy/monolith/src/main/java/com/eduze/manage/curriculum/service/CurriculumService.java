...
public class CurriculumService {
    ...
    public List<StageResponse> listStages() {
        List<CurriculumStage> stages =
                stageMapper.selectList(
                        new LambdaQueryWrapper<CurriculumStage>()
                                .orderByAsc(CurriculumStage::getOrderNo));
        if (stages.isEmpty()) {
            return List.of();
        }
        List<StageDimension> mappings = stageDimensionMapper.selectList(null);
        Map<Long, CurriculumDimension> dimById =
                dimensionMapper.selectList(null).stream()
                        .collect(Collectors.toMap(CurriculumDimension::getId, d -> d));
        Map<Long, List<StageDimension>> byStage =
                mappings.stream().collect(Collectors.groupingBy(StageDimension::getStageId));

        return stages.stream()
                .map(
                        s -> {
                            List<StageResponse.DimensionRefResponse> refs =
                                    byStage.getOrDefault(s.getId(), List.of()).stream()
                                            .map(
                                                    sd -> {
                                                        CurriculumDimension d =
                                                                dimById.get(sd.getDimensionId());
                                                        if (d == null) return null;
                                                        return StageResponse.DimensionRefResponse
                                                                .builder()
                                                                .id(d.getId())
                                                                .code(d.getCode())
                                                                .name(d.getName())
    ...
    public List<DimensionResponse> listDimensions(String kind) {
        LambdaQueryWrapper<CurriculumDimension> wrapper =
                new LambdaQueryWrapper<CurriculumDimension>()
